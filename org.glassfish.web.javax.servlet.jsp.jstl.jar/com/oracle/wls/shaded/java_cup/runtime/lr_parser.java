...
public abstract class lr_parser {
   ...
   public Symbol debug_parse() throws Exception {
      ...
      while(!this._done_parsing) {
         ...
         short var2 = this.get_action(((Symbol)this.stack.peek()).parse_state, this.cur_token.sym);
         if (var2 > 0) {
            this.cur_token.parse_state = var2 - 1;
            ...
            if (var2 == 0) {
               ...
               if (!this.error_recovery(true)) {
                  ...
                  var1 = (Symbol)this.stack.peek();
               }
            }
         } else {
            var1 = this.do_action(-var2 - 1, this, this.stack, this.tos);
            short var3 = this.production_tab[-var2 - 1][0];
            short var4 = this.production_tab[-var2 - 1][1];
            this.debug_reduce(-var2 - 1, var3, var4);
            ...
            var2 = this.get_reduce(((Symbol)this.stack.peek()).parse_state, var3);
            this.debug_message("# Reduce rule: top state " + ((Symbol)this.stack.peek()).parse_state + ", lhs sym " + var3 + " -> state " + var2);
            var1.parse_state = var2;
            var1.used_by_parser = true;
   ...
   protected boolean find_recovery_config(boolean var1) {
      ...
      int var2 = ((Symbol)this.stack.peek()).right;
      int var3 = ((Symbol)this.stack.peek()).left;

      do {
         if (this.shift_under_error()) {
            short var4 = this.get_action(((Symbol)this.stack.peek()).parse_state, this.error_sym());
            if (var1) {
               this.debug_message("# Recover state found (#" + ((Symbol)this.stack.peek()).parse_state + ")");
               this.debug_message("# Shifting on error to state #" + (var4 - 1));
            }

            Symbol var5 = new Symbol(this.error_sym(), var3, var2);
            var5.parse_state = var4 - 1;
            var5.used_by_parser = true;
         ...
         var3 = ((Symbol)this.stack.pop()).left;
         --this.tos;
   ...
   protected final short get_action(int var1, int var2) {
      short[] var3 = this.action_tab[var1];
      int var4;
      if (var3.length < 20) {
         ...
         int var6 = 0;
         int var7 = (var3.length - 1) / 2 - 1;

         while(var6 <= var7) {
            var4 = (var6 + var7) / 2;
            if (var2 == var3[var4 * 2]) {
               return var3[var4 * 2 + 1];
            }
   ...
   protected final short get_reduce(int var1, int var2) {
      short[] var3 = this.reduce_tab[var1];
      if (var3 == null) {
         return -1;
      } else {
   ...
   protected void parse_lookahead(boolean var1) throws Exception {
      ...
      while(true) {
         while(!this._done_parsing) {
            short var3 = this.get_action(((Symbol)this.stack.peek()).parse_state, this.cur_err_token().sym);
            if (var3 > 0) {
               this.cur_err_token().parse_state = var3 - 1;
               ...
               if (var3 == 0) {
                  this.report_fatal_error("Syntax error", var2);
                  return;
               }
            } else {
               var2 = this.do_action(-var3 - 1, this, this.stack, this.tos);
               short var4 = this.production_tab[-var3 - 1][0];
               short var5 = this.production_tab[-var3 - 1][1];
               if (var1) {
                  this.debug_reduce(-var3 - 1, var4, var5);
               }
               ...
               var3 = this.get_reduce(((Symbol)this.stack.peek()).parse_state, var4);
               var2.parse_state = var3;
               var2.used_by_parser = true;
               this.stack.push(var2);
               ++this.tos;
               if (var1) {
                  this.debug_message("# Goto state #" + var3);
               }
            }
         }
   ...
   protected boolean try_parse_ahead(boolean var1) throws Exception {
      virtual_parse_stack var2 = new virtual_parse_stack(this.stack);

      do {
         while(true) {
            short var3 = this.get_action(var2.top(), this.cur_err_token().sym);
            if (var3 == 0) {
               return false;
            }
            ...
            if (var3 > 0) {
               var2.push(var3 - 1);
               if (var1) {
                  this.debug_message("# Parse-ahead shifts Symbol #" + this.cur_err_token().sym + " into state #" + (var3 - 1));
               }
               break;
            }

            if (-var3 - 1 == this.start_production()) {
               if (var1) {
            ...
            if (var1) {
               this.debug_message("# Parse-ahead reduces: handle size = " + var5 + " lhs = #" + var4 + " from state #" + var2.top());
            }

            var2.push(this.get_reduce(var2.top(), var4));
            if (var1) {
               this.debug_message("# Goto state #" + var2.top());
            }
         }
