...
public class Subject extends PolicySchemaObject {
   private static final long serialVersionUID = -2675029937945117521L;
   private List matches;

   public Subject(List matches) {
   ...
   public Subject(AttributeRegistry registry, Node root) throws URISyntaxException, DocumentParseException {
      ...
      for(int i = 0; i < nodes.getLength(); ++i) {
         ...
         if (this.getLocalName(node).equals("SubjectMatch")) {
   ...
   public void encodeChildren(Map nsMap, PrintStream ps) {
      if (this.matches != null) {
         ...
         while(it.hasNext()) {
            SubjectMatch m = (SubjectMatch)it.next();
            m.encode(nsMap, ps);
         }
      }
