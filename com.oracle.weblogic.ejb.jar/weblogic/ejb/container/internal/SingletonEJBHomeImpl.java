...
public class SingletonEJBHomeImpl extends BaseEJBHome implements Ejb3SessionHome, Ejb3RemoteHome {
   ...
   private Remote allocateBI(Class implClass, Class iface) {
      try {
         SingletonRemoteObject sro = new SingletonRemoteObject();
         sro.setEJBHome(this);
         ...
         if (this.getBeanInfo().hasAsyncMethods()) {
            ctr = implClass.getConstructor(SingletonRemoteObject.class, AsyncInvocationManager.class, RMIAsyncInvState.class);
            return (Remote)ctr.newInstance(sro, this.getBeanInfo().getAsyncInvocationManager(), new RMIAsyncInvState());
         } else {
            ctr = implClass.getConstructor(SingletonRemoteObject.class);
   ...
   public void remove(MethodDescriptor md, Handle h) {
      throw new AssertionError("Should not be invoked on Singleton");
   }
