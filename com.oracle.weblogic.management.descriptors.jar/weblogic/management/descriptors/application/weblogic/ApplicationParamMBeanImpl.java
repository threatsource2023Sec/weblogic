...
public class ApplicationParamMBeanImpl extends XMLElementMBeanDelegate implements ApplicationParamMBean {
   ...
   public String toXML(int indentLevel) {
      ...
      result.append(ToXML.indent(indentLevel)).append("<application-param");
      result.append(">\n");
      if (null != this.getDescription()) {
         result.append(ToXML.indent(indentLevel + 2)).append("<description>").append(this.getDescription()).append("</description>\n");
      }

      if (null != this.getParamName()) {
         result.append(ToXML.indent(indentLevel + 2)).append("<param-name>").append(this.getParamName()).append("</param-name>\n");
      }

      if (null != this.getParamValue()) {
         result.append(ToXML.indent(indentLevel + 2)).append("<param-value>").append(this.getParamValue()).append("</param-value>\n");
      }
