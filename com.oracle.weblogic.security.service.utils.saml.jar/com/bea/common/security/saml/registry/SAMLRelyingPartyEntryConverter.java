...
public class SAMLRelyingPartyEntryConverter extends GenericEntryConverter {
   public SAMLRelyingPartyEntryConverter(LoggerSpi logger, LegacyEncryptorSpi encryptor) {
   ...
   public GenericEntryConverter.BusinessObjectInfo getBusinessObjectInfo(String dnString) {
      ...
      if (fieldValues.length != 4) {
         ...
         try {
            BEASAMLRelyingPartyId idObject = new BEASAMLRelyingPartyId(fieldValues[3], fieldValues[2], fieldValues[1], fieldValues[0]);
            this.logDebug("checking id object: " + idObject.toString());
            if (notEmpty(idObject.getCn()) && notEmpty(idObject.getDomainName()) && notEmpty(idObject.getRealmName()) && notEmpty(idObject.getRegistryName()) && idObject.getRegistryName().equalsIgnoreCase("SAMLRelyingPartyRegistry")) {
               return new GenericEntryConverter.BusinessObjectInfo(BEASAMLRelyingParty.class, BEASAMLRelyingPartyId.class, idObject);
            }

            this.logDebug("Not a SAMLRelyingPartyRegistry binding");
   ...
   protected Class determineIdObjectClass(Object businessObject) {
      if (businessObject instanceof BEASAMLRelyingParty) {
         return BEASAMLRelyingPartyId.class;
      } else {
   ...
   public Map buildBaseAttributes(Object businessObjectIdObject) {
      ...
      if (businessObjectIdObject instanceof BEASAMLRelyingPartyId) {
         BEASAMLRelyingPartyId idObject = (BEASAMLRelyingPartyId)businessObjectIdObject;
         ...
         objectClassStrings = new String[]{"top", "beaSAMLRelyingParty", "beaSAMLPartner"};
