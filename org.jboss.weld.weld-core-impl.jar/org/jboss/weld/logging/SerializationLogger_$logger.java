package org.jboss.weld.logging;

import java.io.Serializable;
import java.text.MessageFormat;
import java.util.Arrays;
import org.jboss.logging.BasicLogger;
import org.jboss.logging.DelegatingBasicLogger;
import org.jboss.logging.Logger;
import org.jboss.logging.Logger.Level;
import org.jboss.weld.exceptions.IllegalStateException;
import org.jboss.weld.exceptions.InvalidObjectException;

public class SerializationLogger_$logger extends DelegatingBasicLogger implements SerializationLogger, WeldLogger, BasicLogger, Serializable {
   private static final long serialVersionUID = 1L;
   private static final String FQCN = SerializationLogger_$logger.class.getName();
   private static final String unableToGetBeanIdentifier = "WELD-001800: Unable to get bean identifier at position {0} from {1}";
   private static final String unableToDeserialize = "WELD-001801: Unable to deserialize {0}";
   private static final String catchingDebug = "Catching";

   public SerializationLogger_$logger(Logger log) {
      super(log);
   }

   protected String unableToGetBeanIdentifier$str() {
      return "WELD-001800: Unable to get bean identifier at position {0} from {1}";
   }

   public final IllegalStateException unableToGetBeanIdentifier(int index, Object info) {
      IllegalStateException result = new IllegalStateException(MessageFormat.format(this.unableToGetBeanIdentifier$str(), index, info));
      StackTraceElement[] st = result.getStackTrace();
      result.setStackTrace((StackTraceElement[])Arrays.copyOfRange(st, 1, st.length));
      return result;
   }

   protected String unableToDeserialize$str() {
      return "WELD-001801: Unable to deserialize {0}";
   }

   public final InvalidObjectException unableToDeserialize(Object info, Throwable cause) {
      InvalidObjectException result = new InvalidObjectException(MessageFormat.format(this.unableToDeserialize$str(), info));
      result.initCause(cause);
      StackTraceElement[] st = result.getStackTrace();
      result.setStackTrace((StackTraceElement[])Arrays.copyOfRange(st, 1, st.length));
      return result;
   }

   public final void catchingDebug(Throwable throwable) {
      super.log.logf(FQCN, Level.DEBUG, throwable, this.catchingDebug$str(), new Object[0]);
   }

   protected String catchingDebug$str() {
      return "Catching";
   }
}
