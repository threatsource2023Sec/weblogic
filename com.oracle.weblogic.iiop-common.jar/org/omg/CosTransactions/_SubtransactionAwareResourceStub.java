...
public class _SubtransactionAwareResourceStub extends ObjectImpl implements SubtransactionAwareResource {
   private static String[] __ids = new String[]{"IDL:omg.org/CosTransactions/SubtransactionAwareResource:1.0", "IDL:omg.org/CosTransactions/Resource:1.0"};

   public void commit_subtransaction(Coordinator parent) {
      InputStream $in = null;

      try {
         OutputStream $out = this._request("commit_subtransaction", true);
         CoordinatorHelper.write($out, parent);
         $in = this._invoke($out);
         return;
   ...
   public void rollback_subtransaction() {
      ...
      try {
         OutputStream $out = this._request("rollback_subtransaction", true);
         $in = this._invoke($out);
         return;
