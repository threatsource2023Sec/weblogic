...
final class IANAServicesDB implements ServicesDB {
   ...
   private IANAServicesDB(Map tcpNameToService, Map udpNameToService, Map tcpServices, Map udpServices) {
   ...
   public static final IANAServicesDB getInstance() {
      return IANAServicesDB.SingletonHolder.INSTANCE;
   }

   public final Service getServiceByName(String name, String proto) {
      if ("tcp".equals(proto)) {
         return (Service)this.tcpNameToService.get(name);
      } else if ("udp".equals(proto)) {
         return (Service)this.udpNameToService.get(name);
      } else if (proto == null) {
         Service s = (Service)this.tcpNameToService.get(name);
         return s != null ? s : (Service)this.udpNameToService.get(name);
      } else {
         return null;
      }
   }
   ...
   public final Service getServiceByPort(Integer port, String proto) {
      if ("tcp".equals(proto)) {
         return (Service)this.tcpPortToService.get(port);
      } else if ("udp".equals(proto)) {
         return (Service)this.udpPortToService.get(port);
      } else if (proto == null) {
         Service s = (Service)this.tcpPortToService.get(port);
         return s != null ? s : (Service)this.udpPortToService.get(port);
      } else {
         return null;
      }
   }
   ...
   // $FF: synthetic method
   IANAServicesDB(Map x0, Map x1, Map x2, Map x3, Object x4) {
      this(x0, x1, x2, x3);
   }

   private static final class ServicesBuilder {
      private static final List emptyAliases = Collections.emptyList();
      final Map tcpNameToService;
      final Map udpNameToService;
      final Map tcpPortToService;
      final Map udpPortToService;
      ...
      // $FF: synthetic method
      ServicesBuilder(Object x0) {
         this();
      }
   }

   private static final class SingletonHolder {
      public static final IANAServicesDB INSTANCE = IANAServicesDB.buildServices();
   }
}
