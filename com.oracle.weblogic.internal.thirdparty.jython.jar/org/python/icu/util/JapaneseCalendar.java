...
public class JapaneseCalendar extends GregorianCalendar {
   ...
   protected int handleGetExtendedYear() {
      ...
      if (this.newerField(19, 1) == 19 && this.newerField(19, 0) == 19) {
         ...
         year = this.internalGet(1, 1) + ERAS[this.internalGet(0, CURRENT_ERA) * 3] - 1;
      }
   ...
   protected int getDefaultMonthInYear(int extendedYear) {
      ...
      return extendedYear == ERAS[era * 3] ? ERAS[era * 3 + 1] - 1 : super.getDefaultMonthInYear(extendedYear);
   }

   protected int getDefaultDayInMonth(int extendedYear, int month) {
      int era = this.internalGet(0, CURRENT_ERA);
      return extendedYear == ERAS[era * 3] && month == ERAS[era * 3 + 1] - 1 ? ERAS[era * 3 + 2] : super.getDefaultDayInMonth(extendedYear, month);
   }
   ...
   protected void handleComputeFields(int julianDay) {
      ...
      if (year > ERAS[ERAS.length - 3]) {
         ...
         while(low < high - 1) {
            ...
            if (diff == 0) {
               diff = this.internalGet(2) - (ERAS[i * 3 + 1] - 1);
               if (diff == 0) {
