...
import org.python.icu.impl.Pair;
...
public class MeasureUnit implements Serializable {
   ...
   public boolean equals(Object rhs) {
      if (rhs == this) {
         return true;
      } else if (!(rhs instanceof MeasureUnit)) {
         return false;
      } else {
   ...
   public static synchronized Set getAvailableTypes() {
      populateCache();
      return Collections.unmodifiableSet(cache.keySet());
   }

   public static synchronized Set getAvailable(String type) {
      populateCache();
      Map units = (Map)cache.get(type);
   ...
   public static synchronized Set getAvailable() {
      ...
      Iterator var1 = (new HashSet(getAvailableTypes())).iterator();

      while(var1.hasNext()) {
         String type = (String)var1.next();
         Iterator var3 = getAvailable(type).iterator();

         while(var3.hasNext()) {
            MeasureUnit unit = (MeasureUnit)var3.next();
            result.add(unit);
         }
      }
   ...
   private static void populateCache() {
      if (!cacheIsPopulated) {
         ...
         ICUResourceBundle rb1 = (ICUResourceBundle)UResourceBundle.getBundleInstance("org/python/icu/impl/data/icudt59b/unit", "en");
         rb1.getAllItemsWithFallback("units", new MeasureUnitSink());
         ICUResourceBundle rb2 = (ICUResourceBundle)UResourceBundle.getBundleInstance("org/python/icu/impl/data/icudt59b", "currencyNumericCodes", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
   ...
   protected static synchronized MeasureUnit addUnit(String type, String unitName, Factory factory) {
      Map tmp = (Map)cache.get(type);
      if (tmp == null) {
         cache.put(type, tmp = new HashMap());
      } else {
         type = ((MeasureUnit)((Map.Entry)((Map)tmp).entrySet().iterator().next()).getValue()).type;
      }

      MeasureUnit unit = (MeasureUnit)((Map)tmp).get(unitName);
      if (unit == null) {
         ((Map)tmp).put(unitName, unit = factory.create(type, unitName));
      }
   ...
   private static final class CurrencyNumericCodeSink extends UResource.Sink {
      ...
      // $FF: synthetic method
      CurrencyNumericCodeSink(Object x0) {
         this();
      }
   }
