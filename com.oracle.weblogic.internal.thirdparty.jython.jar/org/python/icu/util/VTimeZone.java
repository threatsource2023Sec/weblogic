...
public class VTimeZone extends BasicTimeZone {
      ...
      if (this.vtzlines != null && this.vtzlines.size() != 0) {
         ...
         do {
            ...
            do {
               if (!var15.hasNext()) {
                  ...
                  if (finalRuleCount == 1) {
                     if (rules.size() == 1) {
                        ...
                        fromOffset = finalRule.getDSTSavings();
                        Date finalStart = finalRule.getFirstStart(initialRawOffset, initialDSTSavings);
                        Date start = finalStart;

                        for(dstSavings = 0; dstSavings < rules.size(); ++dstSavings) {
                           if (finalRuleIdx != dstSavings) {
                              TimeZoneRule r = (TimeZoneRule)rules.get(dstSavings);
                              Date lastStart = r.getFinalStart(tmpRaw, fromOffset);
                  ...
                  Iterator var36 = rules.iterator();

                  while(var36.hasNext()) {
                     rule = (TimeZoneRule)var36.next();
                     rbtz.addTransitionRule(rule);
                  }
               ...
               line = (String)var15.next();
               finalRuleIdx = line.indexOf(":");
            } while(finalRuleIdx < 0);

            String name = line.substring(0, finalRuleIdx);
            String value = line.substring(finalRuleIdx + 1);
            switch (state) {
               case 0:
               ...
               case 2:
                  if (name.equals("DTSTART")) {
                     ...
                     if (dtstart != null && from != null && to != null) {
                        ...
                        rule = null;
                        int fromOffset = false;
                        int toOffset = false;
                        int rawOffset = false;
                        int dstSavings = false;
                        long start = 0L;
         ...
         if (ruleFields == null) {
            ...
            if (dates.size() == 1) {
               if (ruleFields.length > 4) {
                  ...
                  for(earliestDay = 0; earliestDay < 7; ++earliestDay) {
                     days[earliestDay] = ruleFields[3 + earliestDay];
                     days[earliestDay] = days[earliestDay] > 0 ? days[earliestDay] : MONTHLENGTH[month] + days[earliestDay] + 1;
                     earliestMonth = days[earliestDay] < earliestMonth ? days[earliestDay] : earliestMonth;
                  }

                  for(earliestDay = 1; earliestDay < 7; ++earliestDay) {
                     boolean found = false;

                     for(j = 0; j < 7; ++j) {
                        if (days[j] == earliestMonth + earliestDay) {
                           found = true;
                           break;
                        }
                     }
               ...
               label250: {
                  if (month != -1 && dayOfWeek != 0 && dayOfMonth != 0) {
                     ...
                     for(anotherMonth = 0; anotherMonth < daysCount; ++anotherMonth) {
                        j = ruleFields[3 + anotherMonth];
                        j = j > 0 ? j : MONTHLENGTH[month] + j + 1;
                        earliestDay = j < earliestDay ? j : earliestDay;
                     }
            ...
            earliestDay = dfields[5];
            anotherMonth = Integer.MAX_VALUE;
            if (until[0] != Long.MIN_VALUE) {
               Grego.timeToFields(until[0], dfields);
               anotherMonth = dfields[0];
            }

            DateTimeRule adtr = null;
            if (dayOfWeek == 0 && nthDayOfWeek == 0 && dayOfMonth != 0) {
               adtr = new DateTimeRule(month, dayOfMonth, earliestDay, 0);
            } else if (dayOfWeek != 0 && nthDayOfWeek != 0 && dayOfMonth == 0) {
               adtr = new DateTimeRule(month, nthDayOfWeek, dayOfWeek, earliestDay, 0);
            } else {
               if (dayOfWeek == 0 || nthDayOfWeek != 0 || dayOfMonth == 0) {
                  return null;
               }

               adtr = new DateTimeRule(month, dayOfMonth, dayOfWeek, true, earliestDay, 0);
            }

            return new AnnualTimeZoneRule(tzname, rawOffset, dstSavings, adtr, daysCount, anotherMonth);
         }
      } else {
         return null;
      }
         ...
         if (attr.equals("FREQ")) {
            ...
            if (!attr.equals("BYDAY")) {
               if (attr.equals("BYMONTHDAY")) {
                  ...
                  sign = days.countTokens();
                  dayOfMonth = new int[sign];
                  n = 0;

                  while(days.hasMoreTokens()) {
                     try {
                        dayOfMonth[n++] = Integer.parseInt(days.nextToken());
               ...
               if (length >= 2 && length <= 4) {
                  ...
                  for(sign = 0; sign < ICAL_DOW_NAMES.length && !value.equals(ICAL_DOW_NAMES[sign]); ++sign) {
                  }
   ...
   private static TimeZoneRule createRuleByRDATE(String tzname, int rawOffset, int dstSavings, long start, List dates, int fromOffset) {
      ...
      if (dates != null && dates.size() != 0) {
         ...
         String date;
         try {
            for(Iterator var9 = dates.iterator(); var9.hasNext(); times[idx++] = parseDateTimeString(date, fromOffset)) {
               date = (String)var9.next();
            }
   ...
   private static void writeZonePropsByDOW_GEQ_DOM(Writer writer, boolean isDst, String tzname, int fromOffset, int toOffset, int month, int dayOfMonth, int dayOfWeek, long startTime, long untilTime) throws IOException {
      if (dayOfMonth % 7 == 1) {
         ...
         int nextMonth;
         if (dayOfMonth <= 0) {
            nextMonthDays = 1 - dayOfMonth;
            currentMonthDays -= nextMonthDays;
            nextMonth = month - 1 < 0 ? 11 : month - 1;
            writeZonePropsByDOW_GEQ_DOM_sub(writer, nextMonth, -nextMonthDays, dayOfWeek, nextMonthDays, Long.MAX_VALUE, fromOffset);
            startDay = 1;
         } else if (dayOfMonth + 6 > MONTHLENGTH[month]) {
            nextMonthDays = dayOfMonth + 6 - MONTHLENGTH[month];
            currentMonthDays -= nextMonthDays;
   ...
   private static int offsetStrToMillis(String str) {
      ...
      if (!isValid) {
         ...
         length = sign * ((hour * 60 + min) * 60 + sec) * 1000;
         return length;
      }
   }
