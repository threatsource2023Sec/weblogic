...
public class IntTrieBuilder extends TrieBuilder {
   ...
   public IntTrie serialize(TrieBuilder.DataManipulate datamanipulate, Trie.DataManipulate triedatamanipulate) {
      if (datamanipulate == null) {
         ...
         if (this.m_dataLength_ >= 262144) {
            ...
            int options;
            for(options = 0; options < this.m_indexLength_; ++options) {
               index[options] = (char)(this.m_index_[options] >>> 2);
            }
   ...
   public boolean setRange(int start, int limit, int value, boolean overwrite) {
      if (!this.m_isCompacted_ && start >= 0 && start <= 1114111 && limit >= 0 && limit <= 1114112 && start <= limit) {
         if (start == limit) {
            ...
            int repeatBlock;
            if ((start & 31) != 0) {
               rest = this.getDataBlock(start);
               if (rest < 0) {
                  return false;
               }

               repeatBlock = start + 32 & -32;
               if (repeatBlock > limit) {
                  this.fillBlock(rest, start & 31, limit & 31, value, overwrite);
                  return true;
               }

               this.fillBlock(rest, start & 31, 32, value, overwrite);
               start = repeatBlock;
            }
   ...
   private final void fold(TrieBuilder.DataManipulate manipulate) {
      ...
      int indexLength;
      for(indexLength = 1728; indexLength < 1760; ++indexLength) {
         this.m_index_[indexLength] = block;
      }
