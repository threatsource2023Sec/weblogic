...
import org.python.icu.util.AnnualTimeZoneRule;
import org.python.icu.util.BasicTimeZone;
import org.python.icu.util.DateTimeRule;
import org.python.icu.util.InitialTimeZoneRule;
import org.python.icu.util.SimpleTimeZone;
import org.python.icu.util.TimeArrayTimeZoneRule;
import org.python.icu.util.TimeZone;
import org.python.icu.util.TimeZoneRule;
   ...
   private void construct(UResourceBundle top, UResourceBundle res) {
      if (top != null && res != null) {
         ...
         if (this.transitionCount > 0) {
            ...
            int idx = 0;
            if (transPre32 != null) {
               for(ruleRaw = 0; ruleRaw < transPre32.length / 2; ++idx) {
                  this.transitionTimes64[idx] = ((long)transPre32[ruleRaw * 2] & 4294967295L) << 32 | (long)transPre32[ruleRaw * 2 + 1] & 4294967295L;
                  ++ruleRaw;
               }
            }

            if (trans32 != null) {
               for(ruleRaw = 0; ruleRaw < trans32.length; ++idx) {
                  this.transitionTimes64[idx] = (long)trans32[ruleRaw];
                  ++ruleRaw;
               }
            }

            if (transPost32 != null) {
               for(ruleRaw = 0; ruleRaw < transPost32.length / 2; ++idx) {
                  this.transitionTimes64[idx] = ((long)transPost32[ruleRaw * 2] & 4294967295L) << 32 | (long)transPost32[ruleRaw * 2 + 1] & 4294967295L;
                  ++ruleRaw;
               }
            }
         } else {
               ...
               if (local && sec >= transition - 86400L) {
                  ...
                  if (offsetAfter - offsetBefore >= 0) {
                     ...
                     if ((DuplicatedTimeOpt & 3) == 1 && stdToDst || (DuplicatedTimeOpt & 3) == 3 && dstToStd) {
                        transition += (long)offsetBefore;
                     } else if ((DuplicatedTimeOpt & 12) == 4) {
                        transition += (long)offsetBefore;
                     } else {
                        transition += (long)offsetAfter;
                     }
   ...
   private synchronized void initTransitionRules() {
      if (!this.transitionRulesInitialized) {
         ...
         if (this.finalZone != null) {
            ...
            this.firstFinalTZTransition = new TimeZoneTransition(startTime, (TimeZoneRule)prevRule, (TimeZoneRule)firstFinalRule);
         }
