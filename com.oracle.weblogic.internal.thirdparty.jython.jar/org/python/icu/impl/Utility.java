...
public final class Utility {
   ...
   public static final boolean arrayEquals(Object source, Object target) {
      if (source == null) {
         ...
         return arrayEquals((double[])((double[])source), target);
      } else {
         return source instanceof byte[] ? arrayEquals((byte[])((byte[])source), target) : source.equals(target);
      }
   }
   ...
   public static void appendToRule(StringBuffer rule, int c, boolean isLiteral, boolean escapeUnprintable, StringBuffer quoteBuf) {
      if (isLiteral || escapeUnprintable && isUnprintable(c)) {
         ...
         if (quoteBuf.length() > 0) {
            ...
            for(len = 0; quoteBuf.length() >= 2 && quoteBuf.charAt(quoteBuf.length() - 2) == '\'' && quoteBuf.charAt(quoteBuf.length() - 1) == '\''; ++len) {
