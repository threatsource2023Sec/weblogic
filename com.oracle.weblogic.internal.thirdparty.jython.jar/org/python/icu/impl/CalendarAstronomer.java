...
public class CalendarAstronomer {
   ...
   public double getGreenwichSidereal() {
      if (this.siderealTime == Double.MIN_VALUE) {
         double UT = normalize((double)this.time / 3600000.0, 24.0);
   ...
   public double getLocalSidereal() {
      return normalize(this.getGreenwichSidereal() + (double)this.fGmtOffset / 3600000.0, 24.0);
   }
   ...
   private long timeOfAngle(AngleFunc func, double desired, double periodDays, long epsilon, boolean next) {
      ...
      double deltaT = (deltaAngle + (next ? 0.0 : -6.283185307179586)) * periodDays * 8.64E7 / 6.283185307179586;
   ...
   private long riseOrSet(CoordFunc func, boolean rise, double diameter, double refraction, long epsilon) {
      ...
      do {
         pos = func.eval();
         cosD = Math.acos(-tanL * Math.tan(pos.declination));
         psi = ((rise ? 6.283185307179586 - cosD : cosD) + pos.ascension) * 24.0 / 6.283185307179586;
         long newTime = this.lstToUT(psi);
         deltaT = newTime - this.time;
         this.setTime(newTime);
         ++count;
      } while(count < 5 && Math.abs(deltaT) > epsilon);

      cosD = Math.cos(pos.declination);
      psi = Math.acos(Math.sin(this.fLatitude) / cosD);
   ...
   private static final double normPI(double angle) {
      return normalize(angle + Math.PI, 6.283185307179586) - Math.PI;
   }
