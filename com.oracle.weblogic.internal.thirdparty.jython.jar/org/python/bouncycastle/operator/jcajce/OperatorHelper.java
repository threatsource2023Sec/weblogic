...
class OperatorHelper {
   ...
   Signature createSignature(AlgorithmIdentifier var1) throws GeneralSecurityException {
      ...
      if (var1.getAlgorithm().equals(PKCSObjectIdentifiers.id_RSASSA_PSS)) {
         ...
         if (this.notDefaultPSSParams(var3)) {
            try {
               AlgorithmParameters var7 = this.helper.createAlgorithmParameters("PSS");
               var7.init(var3.getEncoded());
               var2.setParameter(var7.getParameterSpec(PSSParameterSpec.class));
            } catch (IOException var5) {
               throw new GeneralSecurityException("unable to process PSS parameters: " + var5.getMessage());
            }
         }
      }
