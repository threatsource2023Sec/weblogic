...
public class BlockLZ4CompressorOutputStream extends CompressorOutputStream {
   ...
   private void expandFromList(byte[] expanded, int offset, int length) {
      ...
      for(int writeOffset = 0; lengthRemaining > 0; writeOffset += copyLen) {
         ...
         if (offsetRemaining <= 0) {
            ...
            byte[] b;
            for(Iterator var9 = this.expandedBlocks.iterator(); var9.hasNext(); blockOffset += b.length) {
               b = (byte[])var9.next();
   ...
   private void clearUnusedPairs() {
      int pairLengths = 0;
      int pairsToKeep = 0;
      Iterator it = this.pairs.descendingIterator();
   ...
   static final class Pair {
      ...
      private void prependTo(Pair other) {
         Iterator listBackwards = this.literals.descendingIterator();
