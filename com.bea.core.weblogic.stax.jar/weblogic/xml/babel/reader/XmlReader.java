...
public final class XmlReader extends Reader {
   ...
   private void useEncodingDecl(PushbackInputStream pb, String encoding) throws IOException {
      ...
      InputStream tempStream = new ByteArrayInputStream(buffer, 4, len);
      Reader r = new InputStreamReader(tempStream, encoding);
      if (r.read() != 108) {
         this.setEncoding(pb, "UTF-8");
         tempStream.close();
      } else {
   ...
   abstract static class BaseReader extends Reader {
      ...
      private static StackPool pool = new StackPool(32);

      BaseReader(InputStream stream) {
         super(stream);
         this.instream = stream;
         this.buffer = (byte[])((byte[])pool.remove());
         if (this.buffer == null) {
