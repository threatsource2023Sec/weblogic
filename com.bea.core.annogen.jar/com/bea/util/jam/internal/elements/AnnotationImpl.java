...
public final class AnnotationImpl extends ElementImpl implements MAnnotation {
   ...
   AnnotationImpl(ElementContext ctx, DefaultAnnotationProxy proxy, String qualifiedName) {
      super(ctx);
      if (proxy == null) {
   ...
   public String getJavadocTagText() {
      JAnnotationValue val = this.getValue("value");
   ...
   public MAnnotation createNestedValue(String name, String annTypeName) {
      if (name == null) {
         ...
         DefaultAnnotationProxy p = this.getContext().createAnnotationProxy(annTypeName);
   ...
   public MAnnotation[] createNestedValueArray(String name, String annComponentTypeName, int dimensions) {
      if (name == null) {
         ...
         for(int i = 0; i < out.length; ++i) {
            DefaultAnnotationProxy p = this.getContext().createAnnotationProxy(annComponentTypeName);
