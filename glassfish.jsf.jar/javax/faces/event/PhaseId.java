...
public class PhaseId implements Comparable {
   ...
   public static PhaseId phaseIdValueOf(String phase) {
      if (null == phase) {
         ...
         if ("ANY".equals(phase)) {
            result = ANY_PHASE;
         } else if ("APPLY_REQUEST_VALUES".equalsIgnoreCase(phase)) {
            result = APPLY_REQUEST_VALUES;
         } else if ("INVOKE_APPLICATION".equalsIgnoreCase(phase)) {
            result = INVOKE_APPLICATION;
         } else if ("PROCESS_VALIDATIONS".equalsIgnoreCase(phase)) {
            result = PROCESS_VALIDATIONS;
         } else if ("RENDER_RESPONSE".equalsIgnoreCase(phase)) {
            result = RENDER_RESPONSE;
         } else if ("RESTORE_VIEW".equalsIgnoreCase(phase)) {
            result = RESTORE_VIEW;
         } else {
            if (!"UPDATE_MODEL_VALUES".equalsIgnoreCase(phase)) {
               throw new FacesException("Not a valid phase [" + phase + "]");
            }

            result = UPDATE_MODEL_VALUES;
         }
