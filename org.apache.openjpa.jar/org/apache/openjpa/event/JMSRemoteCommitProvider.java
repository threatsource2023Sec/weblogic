...
public class JMSRemoteCommitProvider extends AbstractRemoteCommitProvider implements Configurable, GenericConfigurable, ExceptionListener {
   ...
   public void broadcast(RemoteCommitEvent event) {
      try {
         ...
         if (this.log.isTraceEnabled()) {
            this.log.trace(s_loc.get("jms-sent-update", (Object)this._topicName));
         }
      } catch (JMSException var3) {
         if (this.log.isWarnEnabled()) {
            this.log.warn(s_loc.get("jms-send-error", (Object)this._topicName), var3);
         }
      }
      ...
      try {
         if (this._connection != null) {
            ...
            if (this.log.isInfoEnabled()) {
               this.log.info(s_loc.get("jms-close-listener", (Object)this._topicName));
            }
         }
      } catch (JMSException var6) {
         if (this.log.isWarnEnabled()) {
            this.log.warn(s_loc.get("jms-close-error", (Object)this._topicName), var6);
         }
      } finally {
   ...
   protected void connect() {
      try {
         ...
         if (this.log.isInfoEnabled()) {
            this.log.info(s_loc.get("jms-start-listener", (Object)this._topicName));
         }
               ...
               try {
                  ...
                  if (o instanceof RemoteCommitEvent) {
                     if (JMSRemoteCommitProvider.this.log.isTraceEnabled()) {
                        JMSRemoteCommitProvider.this.log.trace(JMSRemoteCommitProvider.s_loc.get("jms-received-update", (Object)JMSRemoteCommitProvider.this._topicName));
                     }
                  ...
                  if (JMSRemoteCommitProvider.this.log.isWarnEnabled()) {
                     JMSRemoteCommitProvider.this.log.warn(JMSRemoteCommitProvider.s_loc.get("jms-receive-error-1"), var10);
                  }
               } finally {
                  if (saveCls != JMSRemoteCommitProvider.this._appClassLoader) {
   ...
   public void onException(JMSException ex) {
      if (this.log.isWarnEnabled()) {
         this.log.warn(s_loc.get("jms-listener-error", (Object)this._topicName), ex);
      }
      ...
      if (this._reconnectAttempts > 0) {
         ...
         for(int i = 0; !connected && i < this._reconnectAttempts; ++i) {
            try {
               ...
               if (this.log.isInfoEnabled()) {
                  this.log.info(s_loc.get("jms-reconnect-fail", (Object)this._topicName), var7);
               }
         ...
         if (!connected && this.log.isErrorEnabled()) {
            ...
            this.log.info(s_loc.get("jms-reconnected", (Object)this._topicName));
         }
