...
public class WL81RemoteMBeanServerProxy implements MBeanServer {
   private MBeanServer _rs;

   public WL81RemoteMBeanServerProxy(MBeanServer rs) {
      this._rs = rs;
   }
   ...
   public void addNotificationListener(ObjectName observed, NotificationListener listener, NotificationFilter filter, Object handback) throws InstanceNotFoundException {
      NotificationListener delegating = new DelegatingNotificationListener(listener);
      this._rs.addNotificationListener(observed, delegating, filter, handback);
   }
   ...
   public static class DelegatingNotificationListener implements RemoteNotificationListener, Serializable {
      private transient NotificationListener _listener;

      public DelegatingNotificationListener(NotificationListener listener) {
         this._listener = listener;
      }

      public void handleNotification(Notification notif, Object obj) {
         if (this._listener != null) {
            this._listener.handleNotification(notif, obj);
         }
