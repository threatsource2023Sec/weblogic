...
public class LockGroupStateComparisonVersionStrategy extends AbstractVersionStrategy {
   public static final String ALIAS = "state-comparison";
   private static final Localizer _loc = Localizer.forPackage(LockGroupStateComparisonVersionStrategy.class);
   ...
   public void insert(OpenJPAStateManager sm, JDBCStore store, RowManager rm) throws SQLException {
      ...
      for(int i = 0; i < fields.length; ++i) {
         if (!fields[i].isPrimaryKey() && fields[i].isVersionable() && ((KodoFieldMapping)fields[i]).getLockGroup() != null) {
            loaded.set(i);
      ...
      if (state != null) {
         ...
         if (sm.isVersionCheckRequired()) {
            ...
            for(int max = loaded.length(); i < max; ++i) {
               if (loaded.get(i) && ((KodoFieldMapping)fields[i]).getLockGroup().isDirty(sm)) {
   ...
   public void afterLoad(OpenJPAStateManager sm, JDBCStore store) {
      ...
      for(int i = 0; i < fields.length; ++i) {
         if (!fields[i].isPrimaryKey() && fields[i].isVersionable() && ((KodoFieldMapping)fields[i]).getLockGroup() != null && sm.getLoaded().get(fields[i].getIndex()) && !loaded.get(i) && !sm.getDirty().get(fields[i].getIndex())) {
            loaded.set(i);
            state[i] = sm.fetch(fields[i].getIndex());
         }
      }
