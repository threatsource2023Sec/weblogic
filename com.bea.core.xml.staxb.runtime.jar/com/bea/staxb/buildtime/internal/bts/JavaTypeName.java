...
public final class JavaTypeName implements Serializable {
   private static final String XMLOBJECT_BEA_CLASSNAME = XmlObject.class.getName();
   ...
   private static final Map cache = Collections.synchronizedMap(new LRUCacheHashMap(1000));

   public static JavaTypeName forString(String className) {
      JavaTypeName retTypeName = (JavaTypeName)cache.get(className);
      if (retTypeName == null) {
         retTypeName = new JavaTypeName(className);
         cache.put(className, retTypeName);
      }
   ...
   private JavaTypeName(String className) {
      if (className == null) {
         ...
         if (className.startsWith("x=")) {
            ...
            className = className.substring(2).intern();
         } else {
         ...
         this.className = className.substring(0, className.length() - 2 * arrayDepth).intern();
         this.arrayString = className.substring(className.length() - 2 * arrayDepth).intern();
      }
   }
   ...
   private JavaTypeName(String nonArrayClassName, String arrayString, boolean isXmlObject) {
      ...
      this.className = nonArrayClassName.intern();
      this.arrayString = arrayString != null ? arrayString.intern() : null;
