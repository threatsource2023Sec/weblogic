...
public final class NameFunction extends StringExpression implements InterrogatingFunction {
   private Expression mArg;
   private Interrogator mInterrogator = null;

   public NameFunction(Expression[] args) throws XPathParsingException {
      if (args.length > 1) {
   ...
   public String evaluateAsString(Context ctx) {
      if (this.mArg == null) {
         String name = this.mInterrogator.getExpandedName(ctx.node);
         return name == null ? "" : name;
      } else {
         List nodeset = this.mArg.evaluateAsNodeset(ctx);
         if (nodeset != null && nodeset.size() != 0) {
            String name = this.mInterrogator.getExpandedName(nodeset.get(0));
            return name == null ? "" : name;
         } else {
            return "";
         }
