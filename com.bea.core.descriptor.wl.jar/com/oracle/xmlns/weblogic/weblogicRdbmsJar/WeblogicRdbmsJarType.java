...
public interface WeblogicRdbmsJarType extends XmlObject {
   SchemaType type = (SchemaType)XmlBeans.typeSystemForClassLoader(WeblogicRdbmsJarType.class.getClassLoader(), "schemacom_bea_xml.system.com_bea_core_descriptor_wl_4_0_0_0").resolveHandle("weblogicrdbmsjartypef9d0type");
   ...
   WeblogicRdbmsRelationType[] getWeblogicRdbmsRelationArray();

   WeblogicRdbmsRelationType getWeblogicRdbmsRelationArray(int var1);

   int sizeOfWeblogicRdbmsRelationArray();

   void setWeblogicRdbmsRelationArray(WeblogicRdbmsRelationType[] var1);

   void setWeblogicRdbmsRelationArray(int var1, WeblogicRdbmsRelationType var2);

   WeblogicRdbmsRelationType insertNewWeblogicRdbmsRelation(int var1);

   WeblogicRdbmsRelationType addNewWeblogicRdbmsRelation();

   void removeWeblogicRdbmsRelation(int var1);

   TrueFalseType getOrderDatabaseOperations();

   boolean isSetOrderDatabaseOperations();

   void setOrderDatabaseOperations(TrueFalseType var1);

   TrueFalseType addNewOrderDatabaseOperations();

   void unsetOrderDatabaseOperations();

   TrueFalseType getEnableBatchOperations();

   boolean isSetEnableBatchOperations();

   void setEnableBatchOperations(TrueFalseType var1);

   TrueFalseType addNewEnableBatchOperations();

   void unsetEnableBatchOperations();

   CreateDefaultDbmsTablesType getCreateDefaultDbmsTables();

   boolean isSetCreateDefaultDbmsTables();

   void setCreateDefaultDbmsTables(CreateDefaultDbmsTablesType var1);

   CreateDefaultDbmsTablesType addNewCreateDefaultDbmsTables();
   ...
   DefaultDbmsTablesDdlType getDefaultDbmsTablesDdl();

   boolean isSetDefaultDbmsTablesDdl();

   void setDefaultDbmsTablesDdl(DefaultDbmsTablesDdlType var1);

   DefaultDbmsTablesDdlType addNewDefaultDbmsTablesDdl();

   void unsetDefaultDbmsTablesDdl();

   CompatibilityType getCompatibility();

   boolean isSetCompatibility();

   void setCompatibility(CompatibilityType var1);

   CompatibilityType addNewCompatibility();
