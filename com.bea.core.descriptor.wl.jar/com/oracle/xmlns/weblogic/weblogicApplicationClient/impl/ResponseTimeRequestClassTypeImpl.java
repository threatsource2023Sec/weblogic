...
public class ResponseTimeRequestClassTypeImpl extends XmlComplexContentImpl implements ResponseTimeRequestClassType {
   ...
   private static final QName GOALMS$2 = new QName("http://xmlns.oracle.com/weblogic/weblogic-application-client", "goal-ms");
   ...
   public XsdIntegerType getGoalMs() {
      synchronized(this.monitor()) {
         ...
         target = (XsdIntegerType)this.get_store().find_element_user(GOALMS$2, 0);
         return target == null ? null : target;
      }
   }

   public void setGoalMs(XsdIntegerType goalMs) {
      this.generatedSetterHelperImpl(goalMs, GOALMS$2, 0, (short)1);
   }

   public XsdIntegerType addNewGoalMs() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         XsdIntegerType target = null;
         target = (XsdIntegerType)this.get_store().add_element_user(GOALMS$2);
         return target;
      }
   }
