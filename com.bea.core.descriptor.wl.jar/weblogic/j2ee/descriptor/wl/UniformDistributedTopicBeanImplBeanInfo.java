...
public class UniformDistributedTopicBeanImplBeanInfo extends UniformDistributedDestinationBeanImplBeanInfo {
   public static final Class INTERFACE_CLASS = UniformDistributedTopicBean.class;

   public UniformDistributedTopicBeanImplBeanInfo(boolean readOnly, String targetVersion) throws IntrospectionException {
      super(readOnly, targetVersion);
   }

   public UniformDistributedTopicBeanImplBeanInfo() throws IntrospectionException {
   }
   ...
   protected BeanDescriptor buildBeanDescriptor() {
      ...
      try {
         beanClass = Class.forName("weblogic.j2ee.descriptor.wl.UniformDistributedTopicBeanImpl");
      ...
      String description = (new String("<p>A uniform distributed topic is a distributed topic whose members are configured as part of its configuration; since members share the same configuration, they are uniform.  There is one member on each JMS Server that conforms to the targeting criteria of the uniform distributed topic.  For example, if a uniform distributed topic is targeted to a cluster, then one member will be created on each JMS server in the cluster.  Members are created and destroyed as the targeting is changed.</p> ")).intern();
      beanDescriptor.setShortDescription(description);
      beanDescriptor.setValue("description", description);
      String[] roleObjectArray = new String[]{BeanInfoHelper.encodeEntities("Deployer")};
      beanDescriptor.setValue("rolesAllowed", roleObjectArray);
      beanDescriptor.setValue("interfaceclassname", "weblogic.j2ee.descriptor.wl.UniformDistributedTopicBean");
   ...
   protected void buildPropertyDescriptors(Map descriptors) throws IntrospectionException {
      ...
      if (!descriptors.containsKey("ForwardingPolicy")) {
         ...
         currentResult = new PropertyDescriptor("ForwardingPolicy", UniformDistributedTopicBean.class, getterName, setterName);
      ...
      if (!descriptors.containsKey("Multicast")) {
         ...
         setterName = null;
         currentResult = new PropertyDescriptor("Multicast", UniformDistributedTopicBean.class, getterName, setterName);
      ...
      if (!descriptors.containsKey("TopicSubscriptionParams")) {
         ...
         setterName = null;
         currentResult = new PropertyDescriptor("TopicSubscriptionParams", UniformDistributedTopicBean.class, getterName, setterName);
