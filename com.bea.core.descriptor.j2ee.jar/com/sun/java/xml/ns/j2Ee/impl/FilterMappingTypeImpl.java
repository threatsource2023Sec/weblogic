...
import com.sun.java.xml.ns.j2Ee.DispatcherType;
...
public class FilterMappingTypeImpl extends XmlComplexContentImpl implements FilterMappingType {
   ...
   public boolean isSetUrlPattern() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         return this.get_store().count_elements(URLPATTERN$2) != 0;
      }
   }
   ...
   public void unsetUrlPattern() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         this.get_store().remove_element(URLPATTERN$2, 0);
      }
   }
   ...
   public ServletNameType getServletName() {
      synchronized(this.monitor()) {
         ...
         target = (ServletNameType)this.get_store().find_element_user(SERVLETNAME$4, 0);
         return target == null ? null : target;
      }
   }
   ...
   public boolean isSetServletName() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         return this.get_store().count_elements(SERVLETNAME$4) != 0;
      }
   }

   public void setServletName(ServletNameType servletName) {
      this.generatedSetterHelperImpl(servletName, SERVLETNAME$4, 0, (short)1);
   }
   ...
   public void unsetServletName() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         this.get_store().remove_element(SERVLETNAME$4, 0);
      }
   }
