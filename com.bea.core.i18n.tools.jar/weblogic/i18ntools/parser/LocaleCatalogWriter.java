...
public final class LocaleCatalogWriter {
   ...
   public static void writeFormattedCatalog(String path, LocaleMessageCatalog catalog) throws IOException {
      writeCatalog(path, catalog, true);
   }

   public static void writeCatalog(String path, LocaleMessageCatalog catalog) throws IOException {
      writeCatalog(path, catalog, false);
   }

   private static void writeCatalog(String catalogPath, LocaleMessageCatalog catalog, boolean formatted) throws IOException {
      ...
      try {
         ...
         if (attrNames.length != attrs.length) {
            throw new IOException("Fatal inconsistency in LocaleMessageCatalog attribute definitions.");
         }
